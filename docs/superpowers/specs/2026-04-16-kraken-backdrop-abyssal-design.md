...
## Build-Time AI Asset Generation (DALL-E 3 + OpenAI TTS)

OpenAI is used **only at build time** to generate baked PNG and audio assets that ship as static files. The runtime stays a plain static site — no API calls from the browser.

### `scripts/generate-art.mjs` — DALL-E 3 image generation

Reads `OPENAI_API_KEY` and `OPENAI_PROJECT_ID` from `.env` (gitignored). Generates and writes to `src/art/`:

| Asset | Purpose | Prompt-style |
|---|---|---|
| `src/art/robot-1.png` | Replace canvas-art robot at left | VEX-competition-style robot silhouette, dark royal-purple, square chassis with mecanum wheels, lift arm, antenna, twin teal-glow camera "eyes," transparent background |
| `src/art/robot-2.png` | Replace canvas-art robot at right | Same family, distinct chassis variant (e.g., longer arm, different sensor placement) |
| `src/art/abyssal-backdrop.png` | Deepest backdrop layer behind procedural shaders | Cinematic deep-ocean abyss, vertical depth gradient, soft caustic hints, distant submerged geometry, color palette `#080010 #1A0033 #6B0AC9 #00E5CC`, no text, no border, square 2048×2048, painterly cinematic style |
| `src/art/accent-jellyfish.png` | Drifting marine accent | Translucent bioluminescent jellyfish silhouette, teal/violet glow, transparent background |
| `src/art/accent-anglerfish.png` | Drifting marine accent | Deep-sea anglerfish silhouette with single bioluminescent lure, dark with teal lure dot, transparent background |
| `src/art/accent-squid.png` | Drifting marine accent | Small deep-sea squid silhouette, royal purple, transparent background |

The script is idempotent: only re-generates a missing asset (so re-running doesn't burn API credits regenerating things). A `--force` flag forces regeneration.

The generated PNGs are **committed to the repo** so the runtime never depends on the API. The `.env` is needed only when re-generating.

### `scripts/generate-tts.mjs` — OpenAI TTS voice accent

Reads `.env`. Generates one voice line for the Kraken Roar moment:

- `src/audio/voice-accent.mp3` — TTS-rendered with a low/raspy voice (e.g., voice `onyx`, low pitch), saying a short sub-2-second phrase like a guttural exhale or a half-whispered `"...pushback..."`. Used at Roar peak as a personality accent under the ambient roar.

Same idempotent-with-`--force` pattern. Audio file committed to repo.

### CC0 ambient audio — `src/audio/`

Hand-curated from freesound.org / BBC SFX library, CC0 or CC-BY (with attribution noted in `src/audio/CREDITS.md`):

| File | Purpose | Source policy |
|---|---|---|
| `src/audio/storm-bed.ogg` | Continuous ambient loop (~30–45s seamless), low-rumbling ocean + distant thunder | CC0 preferred; CC-BY acceptable with credit |
| `src/audio/thunder-crack.ogg` | Sharp impulse for Lightning Strike t=2.8 | CC0 |
| `src/audio/kraken-roar.ogg` | Whale-call / sub-bass animal vocal for Roar moment | CC0 (whale recordings often public domain) |
| `src/audio/low-rumble.ogg` | Sustained sub-bass for moment buildup | CC0 |
| `src/audio/voice-accent.mp3` | Generated by `scripts/generate-tts.mjs` (OpenAI TTS) | OpenAI generated |

The `src/audio/CREDITS.md` file lists every source, license, and original author, so attribution requirements (if any) are satisfied per the licenses.

### `src/audio.js` — rewrite

Replaces the WebAudio synthesis with buffer playback. Same public API (`startStorm`, `startRumble`, `stopRumble`, `thunder`, `growl`) so moments don't change. Internally:

- On `createAudio()`, fetch + decode the five OGG/MP3 files into `AudioBuffer`s. Cache in a map.
- `startStorm()` → loop the storm-bed buffer through a master gain.
- `thunder()` → one-shot the thunder buffer.
- `growl()` → one-shot the kraken-roar buffer + voice-accent buffer (panned slightly left/right, layered).
- `startRumble()` → loop the low-rumble buffer for ~6s with envelope, mirroring the existing pattern.
- `stopRumble()` → fade out + disconnect.

If any audio file fails to load (404), the audio module logs a warning and the moment that would have used it silently no-ops (no crash). The visual flow is unaffected.

### Build-time workflow

```bash
# Re-generate all AI assets (only if missing, or with --force)
node scripts/generate-art.mjs
node scripts/generate-tts.mjs

# CC0 audio is hand-sourced once, committed to src/audio/
# Document each source in src/audio/CREDITS.md
```

The repo ships with all assets pre-generated. A fresh clone needs no API key to run the deployed site.
...
### Group E — AI asset generation scripts (parallel-safe; each agent owns one new file)

- `scripts/generate-art.mjs` — DALL-E 3 generation for 6 PNG assets in `src/art/`
- `scripts/generate-tts.mjs` — OpenAI TTS generation for `src/audio/voice-accent.mp3`
- `src/audio/CREDITS.md` — attribution for CC0 / CC-BY ambient audio

### Group F — Sourced CC0 audio (manual / one-shot)

- `src/audio/storm-bed.ogg` — sourced from freesound.org (CC0)
- `src/audio/thunder-crack.ogg` — sourced from freesound.org (CC0)
- `src/audio/kraken-roar.ogg` — sourced from freesound.org (CC0; whale call processed)
- `src/audio/low-rumble.ogg` — sourced from freesound.org (CC0)

### Group G — `src/audio.js` rewrite (depends on Groups E + F audio assets being present)

- Replace WebAudio synthesis with buffer-source playback
- Same public API surface (`startStorm`, `startRumble`, `stopRumble`, `thunder`, `growl`)
- Graceful no-op if any asset missing
